...
btns.forEach((btn) => {
  btn.addEventListener("click", (e) => {
    let category = e.currentTarget.getAttribute("data-id");
    let filterMenu = menu.filter((items) => {
    ...
    let filteredItems = filterMenu.map((items) => {
      return menuItems(items);
    });
    filteredItems = filteredItems.join("");
    sectionCenter.innerHTML = filteredItems;
  });
});
